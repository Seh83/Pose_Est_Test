...
var exercise_type = "none" //start with the generic exercise

let squatsTopX = null;
let squatsTopY = null;
let squatsBottomX = null;
let squatsBottomY = null;
let squatsReps = 0;
let squatsDisp = 0;
let disp100 = null;
...
/***
 * A custom event listener that listens for exercise change events sent from the flutter backend.
 */
window.addEventListener("changeExercise", (event) => {
    console.log(`Event Skeleton Type: ${event.detail.exercise_type}`)
    setExerciseType(event.detail.exercise_type)
}, false);

function setExerciseType(type) {
    console.log("Exercise Event --->: ", type);
    if (typeof type === 'string' && type.length > 0) {
        exercise_type = type
    }
    else {
        exercise_type = String(type)
    }
}
 ...
 */
function getMetrics(canvasCtx, poses, ctxwidth, ctxheight) {
    //35: mid-hip, 36: mid_knee, 37: mid_ankle,
    let dispVar = null;
    let exercise = {
        name: "squat-front",
        rom: null,
    ...
    if (squatsTopY == null) {
        squatsTopY = poses[35].y;
        squatsTopX = poses[35].x * 0.50;
    } else {
        if (poses[35].y < squatsTopY) {
            squatsTopY = poses[35].y;
            squatsTopX = poses[35].x * 0.50;
        }
    }

    if (squatsBottomY == null) {
        squatsBottomY = (poses[36].y + poses[37].y) * 0.5
        squatsBottomX = poses[35].x * 0.50;
    } else {
        let bottomval = (poses[36].y + poses[37].y) * 0.5
        if (bottomval > squatsBottomY) {
            squatsBottomY = bottomval;
            squatsBottomX = poses[35].x * 0.50;
        }
    }
    ...
    //35: mid-hip, 36: mid_knee, 37: mid_ankle,
    if (disp100 == null) {
        disp100 = Math.sqrt(Math.pow((poses[35].x - poses[37].x), 2) + Math.pow((poses[35].y - poses[37].y), 2));
    } else {
        dispVar = Math.sqrt(Math.pow((poses[35].x - poses[37].x), 2) + Math.pow((poses[35].y - poses[37].y), 2));
    }

    if (disp100 !== null && dispVar !== null) {
        exercise.disp = Math.round((dispVar * 100) / disp100)
    }

    canvasCtx.beginPath();
    canvasCtx.strokeStyle = "white";
    canvasCtx.font = "30px Arial";
    canvasCtx.fillText(Math.abs(100 - exercise.disp) + '%', 50, 50);
    return exercise;
}
...
function updateScreen(results) {
    ...
    switch (skeleton_type) {
        ...
        case "full":
            drawJoints(canvasCtx, results, canvasElement.width, canvasElement.height, "full");
            drawConnections(canvasCtx, results, canvasElement.width, canvasElement.height, "full");
            break;
        case "lumbar":
    ...
    metrics = getMetrics(canvasCtx, results, canvasElement.width, canvasElement.height);
    data.pose = results;
